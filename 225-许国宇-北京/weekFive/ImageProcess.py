...
class Canny:
    ...
    def gradient(self):
        ...
        for r in range(self.gray_iamge.shape[0]):
            for c in range(self.gray_iamge.shape[1]):
    ...
    def nonMaximumSupp(self):
        ...
        for r in range(1,self.img_grad.shape[0]-1):
            for c in range(1,self.img_grad.shape[1]-1):
    ...
    def doubleThresholdDetection(self):
        ...
        for r in range(1,self.img_supp.shape[0]-1):
            for c in range(1,self.img_supp.shape[1]-1):
...
class ImgageProcess(nn.Module):
    ...
    def kmeans(self,img,k):
        ...
        rows,cols=src.shape
        data=src.reshape((rows*cols,1))
        data=np.float32(data)
        criteria=(cv2.TERM_CRITERIA_EPS+cv2.TERM_CRITERIA_MAX_ITER,10,1.0)
        flags=cv2.KMEANS_RANDOM_CENTERS
        compactness,labels,centers=cv2.kmeans(data,k,None,criteria,10,flags)
        target_image=labels.reshape((src.shape[0],src.shape[1]))
        # plt.rcParams['font.sans-serif']=['simHei']
        titles=['source_pic','K-means_pic']
        images=[src,target_image]
        for index in range(2):
            ...
            plt.yticks([])
        plt.show()
    def positionTransform(self,img,img_four_points):
        '''
        transform the angular position of the image
        :param img:the input image
        :param img_four_points:four points from the input image
        :return:the target image
        '''
        src=cv2.imread(img)
        src_copy=src.copy()
        #calculate the desired area size
        distance_c = math.sqrt((img_four_points[1][0] - img_four_points[0][0]) ** 2 + (
                    img_four_points[1][1] - img_four_points[0][1]) ** 2)
        distance_r=math.sqrt((img_four_points[3][0] - img_four_points[0][0]) ** 2 + (
                    img_four_points[3][1] - img_four_points[0][1]) ** 2)

        dst_four_points=np.float32([[0,0],[0,distance_r],[distance_c,0],[distance_c,distance_r]])
        #get the transformation matrix
        transform_matrix=cv2.getPerspectiveTransform(img_four_points,dst_four_points)
        target_image=cv2.warpPerspective(src_copy,transform_matrix,(int(distance_c),int(distance_r)))
        cv2.imshow("src",src)
        cv2.imshow("target_pic",target_image)
        cv2.waitKey(0)
    # edge detection
    ...
    def sample(self,src,mode,dst_size=None,align_corners=None):
        ...
        cv2.imshow(mode,img)
        cv2.imwrite('self_downsample.png',img)
        cv2.waitKey(0)
        return img
    ...
    def gaussSmoothing(self,src,sigma,kernel_size):
        ...
        index=[i-kernel_size//2 for i in range(kernel_size)]
...
if __name__=='__main__':
    imgpros=ImgageProcess()
    self='./data/test_pic/self.jpg'
    lenna='./data/test_pic/lenna.png'
    self_d='./self_downsample.png'
    watch='./data/test_pic/watch.jpg'
    ethan='./data/test_pic/ethan.jpg'
    # imgpros.gray(src)
    # imgpros.sample(src,'nearest',(580,580),None)
    # imgpros.sample(src,'bilinear',(600,600),None)
    # imgpros.sample(src,'area',(1000,1000),None)
    # imgpros.sample(src,'linear',(70![](data/test_pic/watch.jpg)0,700),None)
    ...
    # imgpros.pca(img,2)
    # imgpros.canny(watch,5,0.5)
    # src_points=np.float32([[0,160],[210,645],[380,0],[585,400]])
    # imgpros.positionTransform(watch,src_points)
    imgpros.kmeans(ethan,10)
